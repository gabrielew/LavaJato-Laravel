...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $wash = DB::table('washes')->join('services', 'wash.id_service', '=', 'services.id')
                                   ->select(DB::raw('services.id, services.name, sum(services.price)'))
                                   ->where('washes.created_at > (NOW() - INTERVAL 30 DAY)');
